...
const {EMAIL_HOST, EMAIL_PORT, EMAIL_USER, EMAIL_PASS, FRONT_URL} = process.env;
...
class EmailsService {
    ...
    sendResetPasswordEmail(user, token) {
        return new Promise((resolve, reject) => {
            const tokenLink = `${FRONT_URL}/auth/reset-password/${token}`;
            const htmlBody = `
            ...
            this.sendEmail(user.email, [], [], emailSubject, emailPlainText, htmlBody, (err, response) => {
    ...
    sendAccountValidationEmail(user, token) {
        const tokenLink = `${env.FRONT_URL}/auth/account-validation/${token}`;
        const htmlBody = `
        <table class="es-content" cellspacing="0" cellpadding="0" align="center" bgcolor="#ffffff">
            <tbody>
                <tr>
    ...
    sendEmail(to, cc, bcc, subject, plainText, htmlBody, callback) {

        const mailOptions = {
            from: `"Bedrock - Ne pas répondre" <${EMAIL_USER}>`,
            to,
            cc,
            bcc,
            subject,
